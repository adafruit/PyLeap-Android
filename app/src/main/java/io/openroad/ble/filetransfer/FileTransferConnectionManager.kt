...
interface FileTransferConnectionManager {
    fun reconnect(completion: (isConnected: Boolean) -> Unit)
}

class FakeFileTransferConnectionManagerImpl() : FileTransferConnectionManager {
    override fun reconnect(completion: (isConnected: Boolean) -> Unit) {}

}

class FileTransferConnectionManagerImpl(private val context: Context) :
    FileTransferConnectionManager {

    // Config
    private val reconnectTimeout = 5000
    ...
    override fun reconnect(completion: (isConnected: Boolean) -> Unit) {
        ...
        val pairedAddresses =
            BleManager.getPairedPeripherals(context)?.map { it.address } ?: emptySet()
        val addresses = knownAddresses.intersect(pairedAddresses.toSet())
